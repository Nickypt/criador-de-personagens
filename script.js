...
<script>
    // Seleciona todas as peças de pele para que a cor seja alterada em todas elas
    const skinParts = document.querySelectorAll('#skin-head, #skin-body, #skin-left-arm, #skin-right-arm, #skin-left-leg, #skin-right-leg');

    // Mudar cor da pele
    document.getElementById('skin-color-options').addEventListener('click', (e) => {
    ...
    // Mudar cor do cabelo
    document.getElementById('hair-color-options').addEventListener('click', (e) => {
        const newColor = e.target.dataset.color;
        if (newColor) {
            const currentHair = document.querySelector('#hair-style-1, #hair-style-2');
            if (currentHair) {
                currentHair.style.backgroundColor = newColor;
            }
        }
    });

    // Mudar estilo do cabelo
    document.getElementById('hair-style-options').addEventListener('click', (e) => {
        const newStyleId = e.target.dataset.style;
        if (newStyleId) {
            document.querySelectorAll('#hair-style-1, #hair-style-2').forEach(style => {
                style.classList.add('hidden');
            });
            document.getElementById(newStyleId).classList.remove('hidden');
        }
    });
    
    // Mudar cor dos olhos
    document.getElementById('eye-color-options').addEventListener('click', (e) => {
        const newColor = e.target.dataset.color;
        if (newColor) {
            document.getElementById('left-iris').style.backgroundColor = newColor;
            document.getElementById('right-iris').style.backgroundColor = newColor;
        }
    });

    // Mudar estilo da camisa
    document.getElementById('shirt-style-options').addEventListener('click', (e) => {
        const newStyleId = e.target.dataset.style;
        if (newStyleId) {
            document.querySelectorAll('#shirt-style-1, #shirt-style-2').forEach(style => {
                style.classList.add('hidden');
            });
            document.getElementById(newStyleId).classList.remove('hidden');
        }
    ...
    // Mudar cor da camisa
    document.getElementById('shirt-color-options').addEventListener('click', (e) => {
        const newColor = e.target.dataset.color;
        if (newColor) {
            const currentShirt = document.querySelector('#shirt-style-1:not(.hidden), #shirt-style-2:not(.hidden)');
            if (currentShirt) {
                currentShirt.style.backgroundColor = newColor;
            }
        }
    });
    
    // Mudar estilo das calças
    document.getElementById('pants-style-options').addEventListener('click', (e) => {
        const newStyleId = e.target.dataset.style;
        if (newStyleId) {
            document.querySelectorAll('#pants-style-1, #pants-style-2').forEach(style => {
                style.classList.add('hidden');
            });
            document.getElementById(newStyleId).classList.remove('hidden');
        }
    ...
    // Mudar cor das calças
    document.getElementById('pants-color-options').addEventListener('click', (e) => {
        const newColor = e.target.dataset.color;
        if (newColor) {
            const currentPants = document.querySelector('#pants-style-1:not(.hidden), #pants-style-2:not(.hidden)');
            if (currentPants) {
                currentPants.style.backgroundColor = newColor;
            }
        }
    });

    // Lidar com acessórios
    document.getElementById('accessory-options').addEventListener('click', (e) => {
